function [] = initialization_copy(inp,gamma,f,gmm_comps,thresh1)
sz = size(inp);
thresh = 0.00001;
old_energy = inf;
...
for y = 1:szb(1)
    for x = 1:szb(2)
        ...
        g = inp_crop(y, x, 2);
        b = inp_crop(y, x, 3);
        curr_pixel = double([r g b]);
       
        if x < szb(2) 
            r = inp_crop(y, x+1, 1);
            g = inp_crop(y, x+1, 2);
            b = inp_crop(y, x+1, 3);
            nex_pixel_r = double([r g b]);
            betaaa = betaaa+norm(curr_pixel-nex_pixel_r)^2;
            timer = timer+1;
        end
        ...
        if y < szb(1) 
            ...
            g = inp_crop(y+1, x, 2);
            b = inp_crop(y+1, x, 3);
            nex_pixel_d = double([r g b]);
            betaaa = betaaa+norm(curr_pixel-nex_pixel_d)^2;
            timer = timer+1;
        end
    end
end
    ...
    for x = 1:sze(2)
        ...
        pixel = double([r g b]);
         ...
         if y < sze(1)
            ...
            g = inp_crop(y+1, x, 2);
            b = inp_crop(y+1, x, 3);
            pixel_d = double([r g b]);
            tempo = gamma*exp(-beta*(norm(pixel-pixel_d)^2));
            yo = gamma*exp(-beta*temp3(y,x));
            tempo = yo;
        ...
        if x < sze(2)
            ...
            g = inp_crop(y, x+1, 2);
            b = inp_crop(y, x+1, 3);
            pixel_r = double([r g b]);
            tempo = gamma*exp(-beta*(norm(pixel-pixel_r)^2));
            yo = gamma*exp(-beta*temp4(y,x));
            tempo = yo;
